...
class ChatClient:
    ...
    def setup_login_ui(self):
        ...
        ttk.Label(self.login_frame, text="欢迎来到聊天室", font=(self.fnt, 18, 'bold')).pack(pady=20)
        
        ttk.Label(self.login_frame, text="用户名:", font=(self.fnt, 11)).pack(pady=5)
        self.username_entry = ttk.Entry(self.login_frame, width=30, font=(self.fnt, 11))
        self.username_entry.pack(pady=5)
        self.username_entry.focus()
        
        ttk.Label(self.login_frame, text="服务器地址:", font=(self.fnt, 11)).pack(pady=5)
        self.host_entry = ttk.Entry(self.login_frame, width=30, font=(self.fnt, 11))
        self.host_entry.insert(0, "127.0.0.1")
        self.host_entry.pack(pady=5)
        
        ttk.Label(self.login_frame, text="端口:", font=(self.fnt, 11)).pack(pady=5)
        self.port_entry = ttk.Entry(self.login_frame, width=30, font=(self.fnt, 11))
        self.port_entry.insert(0, "55555")
        self.port_entry.pack(pady=5)
        
        ttk.Button(self.login_frame, text="连接", command=self.connect_to_server).pack(pady=20)
    ...
    def setup_chat_ui(self):
        ...
        ttk.Label(left_frame, text="在线用户", font=(self.fnt, 11, 'bold')).pack(pady=5)
        ...
        # 发送按钮
        ttk.Button(bottom_frame, text="发送", command=self.send_message).pack(side=tk.RIGHT, padx=2)
        
        # 文件按钮
        ttk.Button(bottom_frame, text="📎 文件", command=self.send_file).pack(side=tk.RIGHT, padx=2)
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            self.root.title(f"网络聊天室 - {username}")
            self.setup_chat_ui()
            ...
            self.display_message(f"成功连接到服务器 {host}:{port}", 'system')
            
        except socket.timeout:
            messagebox.showerror("连接超时", "连接服务器超时，请检查服务器地址和端口")
        except ConnectionRefusedError:
            messagebox.showerror("连接被拒绝", "服务器拒绝连接，请检查服务器是否启动")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if not data:
                        self.display_message("与服务器断开连接", 'system')
                        break
                except socket.timeout:
                    continue
                except Exception as e:
                    self.display_message(f"接收数据错误: {str(e)}", 'system')
                    break
                ...
                if msg_data['type'] == 'user_list':
                    ...
                    self.display_message(self.lang_dict['zh'][msg_data['lang_key']].format(msg_data['fmt']), 'system', msg_data.get('time'))
                    ...
                    self.display_message(f"[私聊] {msg_data['from']}: {msg_data['message']}", 
                    ...
                    try:
                        ...
                        if len(size_bytes) == 8:
                            ...
                            expecting_file = True
                            self.display_message(f"收到文件: {msg_data['filename']} ({expected_file_size} bytes)", 
                                               'file', msg_data['time'])
                        else:
                            self.display_message("文件大小信息错误", 'system')
                    except Exception as e:
                        self.display_message(f"文件接收准备错误: {str(e)}", 'system')
                    
            except Exception as e:
                if self.client:
                    self.display_message(f"连接错误: {str(e)}", 'system')
                break
        ...
        try:
            ...
            if msg_type == 'public' and username:
                if username == self.username:
                    self.chat_display.insert(tk.END, "我", 'own_message')
                else:
        ...
        try:
            if message.startswith('/msg '):
                ...
                if len(parts) >= 3:
                    ...
                    self.display_message("用法: /msg 用户名 消息内容", 'system')
            else:
                data = json.dumps({
                    'type': 'public',
                    'message': message
    ...
    def start_private_chat(self, event=None):
        ...
        if selection:
            ...
            target = user_text.replace("👤 ", "").replace(" (我)", "")
    ...
    def send_private_message(self, target, message):
        ...
        try:
            self.client.send(data.encode('utf-8'))
            self.display_message(f"[私聊 → {target}] {message}", 'private', 
    ...
    def send_file(self):
        ...
        if selection:
            ...
            if target == self.username:
                target = None
                if not messagebox.askyesno("发送文件", "未选择用户，发送给所有人?"):
                    return
        ...
        try:
            ...
            if target:
                self.display_message(f"文件已发送给 {target}: {filename} ({file_size} bytes)", 'file', 
                                   datetime.now().strftime('%H:%M:%S'))
            else:
                self.display_message(f"文件已广播: {filename} ({file_size} bytes)", 'file', 
                                   datetime.now().strftime('%H:%M:%S'))
    ...
    def save_received_file(self, file_info, file_data):
        ...
        try:
            with open(save_name, 'wb') as f:
                f.write(file_data)
            self.display_message(f"文件已保存: {save_name} ({len(file_data)} bytes)", 'file')
        except Exception as e:
            self.display_message(f"文件保存失败: {str(e)}", 'system')
