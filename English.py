...
class ChatClient:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Online Chatroom")
    ...
    def setup_login_ui(self):
        ...
        ttk.Label(self.login_frame, text="Welcome to the Online Chatroom", font=(self.fnt, 18, 'bold')).pack(pady=20)
        
        ttk.Label(self.login_frame, text="Username:", font=(self.fnt, 11)).pack(pady=5)
        self.username_entry = ttk.Entry(self.login_frame, width=30, font=(self.fnt, 11))
        self.username_entry.pack(pady=5)
        self.username_entry.focus()
        
        ttk.Label(self.login_frame, text="Server IP:", font=(self.fnt, 11)).pack(pady=5)
    ...
    def setup_chat_ui(self):
        ...
        ttk.Label(left_frame, text="Online Users", font=(self.fnt, 11, 'bold')).pack(pady=5)
        ...
        # 发送按钮
        ttk.Button(bottom_frame, text="Send", command=self.send_message).pack(side=tk.RIGHT, padx=2)
        
        # 文件按钮
        ttk.Button(bottom_frame, text="📎 File", command=self.send_file).pack(side=tk.RIGHT, padx=2)
    ...
    def connect_to_server(self):
        ...
        if not username:
            messagebox.showerror("Error", "Please input the USERNAME")
            return
        ...
        try:
            ...
            self.root.title(f"Online Chatroom - {username}")
            self.setup_chat_ui()
            ...
            self.display_message(f"Successfully connected to the server {host}:{port}", 'system')
            
        except socket.timeout:
            messagebox.showerror("Time out", "Connect time out，Please check the SERVER IP and PORT")
        except ConnectionRefusedError:
            messagebox.showerror("Connection Refused", "The server refused to connect,Please check if the server works")
        except Exception as e:
            messagebox.showerror("Error in connecting", f"Unable to connect the server: {str(e)}")
            if self.client:
                try:
                    self.client.close()
                except:
        ...
        while True:
            try:
                ...
                if expecting_file:
                    while len(file_data_buffer) < expected_file_size:
                        try:
                            ...
                            if not chunk:
                                self.display_message("Error while transporting file", 'system')
                                break
                            file_data_buffer += chunk
                        except socket.timeout:
                            continue
                        except Exception as e:
                            self.display_message(f"Error while receiving file : {str(e)}", 'system')
                            break
                    
                    if len(file_data_buffer) == expected_file_size:
                        self.save_received_file(file_info, file_data_buffer)
                    else:
                        self.display_message(f"The file is incomplete", 'system')
                ...
                try:
                    ...
                    if not data:
                        self.display_message("The connection to the server breaks", 'system')
                        break
                except socket.timeout:
                    continue
                except Exception as e:
                    self.display_message(f"Error while receiving messages: {str(e)}", 'system')
                    break
                ...
                if msg_data['type'] == 'user_list':
                    ...
                    self.display_message(self.lang_dict['en'][msg_data['lang_key']].format(msg_data['fmt']), 'system', msg_data.get('time'))
                    ...
                    self.display_message(f"[Private] {msg_data['from']}: {msg_data['message']}", 
                    ...
                    try:
                        ...
                        if len(size_bytes) == 8:
                            ...
                            expecting_file = True
                            self.display_message(f"Received file: {msg_data['filename']} ({expected_file_size} bytes)", 
                                               'file', msg_data['time'])
                        else:
                            self.display_message("Error file size", 'system')
                    except Exception as e:
                        self.display_message(f"Error while getting ready to receiving file: {str(e)}", 'system')
                    
            except Exception as e:
                if self.client:
                    self.display_message(f"Connection Error: {str(e)}", 'system')
                break
        ...
        try:
            ...
            if msg_type == 'public' and username:
                if username == self.username:
                    self.chat_display.insert(tk.END, "Me", 'own_message')
                else:
            ...
            print(f"Error in showing messages: {e}")
    ...
    def update_user_list(self, users):
        ...
        try:
            ...
            print(f"Error in updating users: {e}")
    ...
    def send_message(self, event=None):
        ...
        try:
            ...
            self.display_message(f"Receiving failed: {str(e)}", 'system')
    ...
    def start_private_chat(self, event=None):
        ...
        if selection:
            ...
            target = user_text.replace("👤 ", "").replace(" (Me)", "")
    ...
    def send_private_message(self, target, message):
        ...
        try:
            self.client.send(data.encode('utf-8'))
            self.display_message(f"[Private → {target}] {message}", 'private', 
                               datetime.now().strftime('%H:%M:%S'))
        except Exception as e:
            self.display_message(f"发送失败: {str(e)}", 'system')
    ...
    def send_file(self):
        ...
        if selection:
            ...
            if target == self.username:
                target = None
                if not messagebox.askyesno("Receiving File", "You haven't choose any users yet, send to ALL USERS?"):
                    return
