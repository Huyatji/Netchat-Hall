...
class ChatClient:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Salon de discussion en ligne")
    ...
    def setup_login_ui(self):
        ...
        ttk.Label(self.login_frame, text="Bienvenue dans le salon", font=(self.fnt, 18, 'bold')).pack(pady=20)
        
        ttk.Label(self.login_frame, text="Nom d'utilisateur:", font=(self.fnt, 11)).pack(pady=5)
        self.username_entry = ttk.Entry(self.login_frame, width=30, font=(self.fnt, 11))
        self.username_entry.pack(pady=5)
        self.username_entry.focus()
        
        ttk.Label(self.login_frame, text="Adresse du serveur:", font=(self.fnt, 11)).pack(pady=5)
        self.host_entry = ttk.Entry(self.login_frame, width=30, font=(self.fnt, 11))
        self.host_entry.insert(0, "127.0.0.1")
        self.host_entry.pack(pady=5)
        
        ttk.Label(self.login_frame, text="Port:", font=(self.fnt, 11)).pack(pady=5)
        self.port_entry = ttk.Entry(self.login_frame, width=30, font=(self.fnt, 11))
        self.port_entry.insert(0, "55555")
        self.port_entry.pack(pady=5)
        
        ttk.Button(self.login_frame, text="Connexion", command=self.connect_to_server).pack(pady=20)
        
        # Lier la touche Entrée au bouton de connexion
        self.root.bind('<Return>', lambda e: self.connect_to_server())
    
    def setup_chat_ui(self):
        """Configurer l'interface de discussion"""
        # Détruire l'interface de connexion
        self.login_frame.destroy()
        
        # ⭐ Détacher la touche Entrée de la connexion
        ...
        ttk.Label(left_frame, text="Utilisateurs en ligne", font=(self.fnt, 11, 'bold')).pack(pady=5)
        ...
        # Configurer les styles de messages
        ...
        # ⭐ Lier la touche Entrée du champ de message à l'envoi
        self.message_entry.bind('<Return>', self.send_message)
        
        # Bouton d'envoi
        ttk.Button(bottom_frame, text="Envoyer", command=self.send_message).pack(side=tk.RIGHT, padx=2)
        
        # Bouton fichier
        ttk.Button(bottom_frame, text="📎 Fichier", command=self.send_file).pack(side=tk.RIGHT, padx=2)
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            self.root.title(f"Salon de discussion en ligne - {username}")
            self.setup_chat_ui()
            ...
            self.display_message(f"Connecté au serveur {host}:{port}", 'system')
            
        except socket.timeout:
            messagebox.showerror("Délai dépassé", "Délai de connexion dépassé, vérifiez l'adresse et le port")
        except ConnectionRefusedError:
            messagebox.showerror("Connexion refusée", "Le serveur a refusé la connexion, vérifiez s'il est démarré")
        except Exception as e:
            messagebox.showerror("Échec de connexion", f"Impossible de se connecter : {str(e)}")
            if self.client:
                try:
                    self.client.close()
                except:
        ...
        while True:
            try:
                ...
                if expecting_file:
                    while len(file_data_buffer) < expected_file_size:
                        try:
                            ...
                            if not chunk:
                                self.display_message("Transfert de fichier interrompu", 'system')
                                break
                            file_data_buffer += chunk
                        except socket.timeout:
                            continue
                        except Exception as e:
                            self.display_message(f"Erreur de réception fichier : {str(e)}", 'system')
                            break
                    
                    if len(file_data_buffer) == expected_file_size:
                        self.save_received_file(file_info, file_data_buffer)
                    else:
                        self.display_message("Fichier reçu incomplet", 'system')
                ...
                try:
                    ...
                    if not data:
                        self.display_message("Déconnecté du serveur", 'system')
                        break
                except socket.timeout:
                    continue
                except Exception as e:
                    self.display_message(f"Erreur de réception : {str(e)}", 'system')
                    break
                ...
                if msg_data['type'] == 'user_list':
                    ...
                    self.display_message(self.lang_dict['fr'][msg_data['lang_key']].format(msg_data['fmt']), 'system', msg_data.get('time'))
                    ...
                    self.display_message(f"[Privé] {msg_data['from']}: {msg_data['message']}", 
                    ...
                    try:
                        ...
                        if len(size_bytes) == 8:
                            ...
                            expecting_file = True
                            self.display_message(f"Fichier reçu : {msg_data['filename']} ({expected_file_size} bytes)", 
                                               'file', msg_data['time'])
                        else:
                            self.display_message("Erreur d'information sur la taille du fichier", 'system')
                    except Exception as e:
                        self.display_message(f"Erreur de préparation fichier : {str(e)}", 'system')
                    
            except Exception as e:
                if self.client:
                    self.display_message(f"Erreur de connexion : {str(e)}", 'system')
                break
        ...
        try:
            ...
            if msg_type == 'public' and username:
                if username == self.username:
                    self.chat_display.insert(tk.END, "Moi", 'own_message')
                else:
    ...
    def update_user_list(self, users):
        ...
        try:
            ...
            print(f"Erreur de mise à jour liste utilisateurs : {e}")
        ...
        try:
            if message.startswith('/msg '):
                ...
                if len(parts) >= 3:
                    ...
                    self.display_message("Utilisation : /msg nom_utilisateur message", 'system')
            else:
                data = json.dumps({
                    'type': 'public',
                    'message': message
            ...
            self.display_message(f"Échec d'envoi : {str(e)}", 'system')
    ...
    def start_private_chat(self, event=None):
        ...
        if selection:
            ...
            target = user_text.replace("👤 ", "").replace(" (Moi)", "")
    ...
    def send_private_message(self, target, message):
        ...
        try:
            self.client.send(data.encode('utf-8'))
            self.display_message(f"[Privé → {target}] {message}", 'private', 
    ...
    def send_file(self):
        ...
        if selection:
            ...
            if target == self.username:
                target = None
                if not messagebox.askyesno("Envoyer un fichier", "Aucun utilisateur sélectionné, envoyer à tout le monde ?"):
                    return
        ...
        try:
            ...
            if target:
                self.display_message(f"Fichier envoyé à {target} : {filename} ({file_size} bytes)", 'file', 
                                   datetime.now().strftime('%H:%M:%S'))
            else:
                self.display_message(f"Fichier diffusé : {filename} ({file_size} bytes)", 'file', 
                                   datetime.now().strftime('%H:%M:%S'))
            
        except Exception as e:
            messagebox.showerror("Échec d'envoi du fichier", str(e))
    ...
    def save_received_file(self, file_info, file_data):
        ...
        try:
            with open(save_name, 'wb') as f:
                f.write(file_data)
            self.display_message(f"Fichier sauvegardé : {save_name} ({len(file_data)} bytes)", 'file')
        except Exception as e:
            self.display_message(f"Échec de sauvegarde : {str(e)}", 'system')
